...
/* Exposes: TransactionsPage, DuplicatePage */

const { useState, useMemo, useCallback } = React;

const fmt = n => n != null && n !== '' ? Number(n).toLocaleString('th-TH', {minimumFractionDigits:2}) : '—';
...
const TransactionsPage = ({ transactions = [] }) => {
  ...
  const types = useMemo(() => [...new Set(transactions.map(t => t.tx_type).filter(Boolean))].sort(), [transactions]);

  const filtered = useMemo(() => {
    const ql = q.toLowerCase();
    return transactions.filter(t => {
      if (q && ![t.from_account_no,t.to_account_no,t.transaction_ref,
                  t.from_account_name,t.to_account_name,t.raw_description,
                  t.tx_type,t.channel].some(v => v && String(v).toLowerCase().includes(ql))) return false;
  ...
  const totalIn  = filtered.filter(t=>t.tx_direction==='IN' &&['UNIQUE','MASTER'].includes(t.duplicate_status))
                           .reduce((s,t)=>s+(Number(t.amount)||0),0);
  const totalOut = filtered.filter(t=>t.tx_direction==='OUT'&&['UNIQUE','MASTER'].includes(t.duplicate_status))
                           .reduce((s,t)=>s+(Number(t.amount)||0),0);
  ...
  return (
    <div>
      <Topbar title="📊 Transactions">
        ...
        <span className="text-sm text-green ml-auto">IN: {fmt(totalIn)}</span>
        <span className="text-sm text-red">OUT: {fmt(totalOut)}</span>
      </Topbar>
        ...
        {transactions.length === 0 ? (
          ...
          <>
            ...
            <div className="filter-bar">
              ...
              <input className="form-input" placeholder="Search account / ref / name…"
                value={q} onChange={e=>{setQ(e.target.value);setPage(1);}} style={{minWidth:220}}/>
                  ...
                  <tbody>
                    {paged.map(tx => [
                      <tr key={tx.txid}
                        ...
                        style={{cursor:'pointer',opacity: tx.duplicate_status==='DUPLICATE'?0.55:1}}>
                        <td>{tx.tx_date||'—'}</td>
                        ...
                        <td className="mono text-xs">{fmt(tx.balance)}</td>
                        <td>
                          <div className="mono text-xs">{tx.from_account_no||'—'}</div>
                          {tx.from_account_name && <div className="text-xs text-muted truncate" style={{maxWidth:140}}>{tx.from_account_name}</div>}
                        </td>
                        <td>
                          <div className="mono text-xs">{tx.to_account_no||'—'}</div>
                          {tx.to_account_name && <div className="text-xs text-muted truncate" style={{maxWidth:140}}>{tx.to_account_name}</div>}
                        </td>
...
const TxDetail = ({ tx }) => {
  const groups = [
    ...
    { label:'From', fields:['from_bank_code','from_bank_name','from_account_no','from_account_name'] },
    { label:'To',   fields:['to_bank_code','to_bank_name','to_account_no','to_account_name'] },
  ...
  return (
    <div style={{padding:'12px 0',display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:16}}>
      {groups.map(g => {
        const vals = g.fields.filter(f => tx[f] !== undefined && tx[f] !== '' && tx[f] !== null);
          ...
          <div key={g.label} style={{background:'var(--surface3)',borderRadius:6,padding:'10px 14px'}}>
            ...
            {vals.map(f => (
              <div key={f} style={{display:'flex',gap:8,marginBottom:4,fontSize:11,alignItems:'flex-start'}}>
                ...
                <span className="mono" style={{wordBreak:'break-all',color:'var(--text)'}}>
...
const DuplicatePage = ({ transactions = [], onTransactionsUpdated }) => {
  ...
  const [filter, setFilter] = useState('all'); // all, exact, possible, review

  const dupGroups = useMemo(() => {
    const groups = {};
    for (const tx of transactions) {
      if (tx.duplicate_status === 'UNIQUE' || tx.duplicate_status === 'MERGED') continue;
      const key = tx.duplicate_group_id;
      if (!key) continue;
      if (!groups[key]) groups[key] = [];
      groups[key].push(tx);
    }
    return Object.values(groups).sort((a,b) => {
      const order = {POSSIBLE_DUPLICATE:0,DUPLICATE:1,MASTER:2};
      return (order[a[0]?.duplicate_status]||9) - (order[b[0]?.duplicate_status]||9);
    });
  }, [transactions]);
  ...
  return (
    <div>
      <Topbar title="🔁 Duplicate Review">
        <span className="text-sm text-muted">{total} groups</span>
      </Topbar>
      <div className="page-content">
        ...
        {total === 0 ? (
          ...
          <>
            <div className="stats-grid mb-16">
              <StatCard value={total}         label="Total Groups"/>
            ...
            <div className="flex items-center gap-8 mb-16">
              {[['all','All'],['exact','Exact Dup'],['possible','Possible'],['review','Review']].map(([v,l])=>(
...
const DupGroup = ({ group, onMerge, onSeparate }) => {
  ...
  return (
    <div className="card" style={{borderColor: isPossible?'rgba(246,173,85,.3)':'rgba(245,101,101,.3)'}}>
      {/* Header */}
      <div className="flex items-center gap-12" style={{cursor:'pointer'}} onClick={()=>setOpen(o=>!o)}>
        ...
        {isPossible && group[0].confidence_score && (
          <span className="text-xs text-amber">Confidence: {Math.round(group[0].confidence_score*100)}%</span>
        )}
        <span className="text-sm text-muted ml-auto">{group[0].review_reason || ''}</span>
        <Icon name="chevron" size={14} style={{transform:open?'rotate(90deg)':'none',transition:'transform .2s'}}/>
      </div>
            ...
            {group.map((tx, i) => (
              <div key={tx.txid} className={`dup-side ${i===0?'master':'dupe'}`}>
                <div className="flex items-center gap-8 mb-8">
                  ...
                  <span className="text-xs text-muted">{tx.source_file_name}</span>
                </div>
                {[
                  ...
                  ['Balance', fmt(tx.balance)],
                  ['From Acc', tx.from_account_no],
                  ['From Name', tx.from_account_name],
                  ['To Acc', tx.to_account_no],
                  ...
                  <div key={l} style={{display:'flex',gap:8,marginBottom:4,fontSize:11}}>
                    ...
                    <span className="mono">{String(v).length>60?String(v).slice(0,60)+'…':v}</span>
                  </div>
                ) : null)}
              </div>
            ))}
          ...
          <div className="flex items-center gap-8 mt-12">
            {isPossible ? (
              ...
              <span className="text-xs text-muted">
                รายการนี้ถูก Merge อัตโนมัติแล้ว (Exact duplicate) ·
